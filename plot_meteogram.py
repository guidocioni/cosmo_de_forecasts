...
for city in cities:# This works regardless if cities is either single value or array
    ...
    WW = xr.open_mfdataset(folder+'cosmo-d2_single-level_WW_*').sel(lon=lon, lat=lat, method='nearest')['WW'].load()
    weather_icons = get_weather_icons(WW, time_local)
    ...
    for dt, weather_icon, dewp in zip(time_local, weather_icons, dset_city['2t']):
