...
{
	filename="cosmo-d2_germany_regular-lat-lon_time-invariant_${year}${month}${day}${run}_HSURF.grib2"
	wget -r -nH -np -nv -nd --reject "index.html*" --cut-dirs=3 -A "${filename}.bz2" "https://opendata.dwd.de/weather/nwp/cosmo-d2/grib/${run}/hsurf/"
	bzip2 -d ${filename}.bz2 
	cdo -f nc copy ${filename} HSURF_${year}${month}${day}${run}_de.nc
	rm ${filename}
}
export -f download_invariant_cosmo_d2
